...
Public Class Form172
    ...
    Dim StartWeek As String = String.Empty '起始周
    Dim EndWeek As String = String.Empty   '結束周
    Dim CurrentWeek As String = String.Empty   '當前週
    Dim xExcel As Microsoft.Office.Interop.Excel.Application
    Dim xWorkBook As Microsoft.Office.Interop.Excel.Workbook
    Dim Ws As Microsoft.Office.Interop.Excel.Worksheet
    Dim oRng As Microsoft.Office.Interop.Excel.Range
    Dim oRng1 As Microsoft.Office.Interop.Excel.Range
    Dim EQindex As Int16 = 0        'P1 指標
    Dim ShipmentIndex As Int16 = 0  'P4 指標
    Dim LineZ As Integer = 0
    Dim g_Success As Boolean = True
    Dim BiasWeek As Int16 = 0
    Dim ReportYear As Int16 = 0
    Dim AutoClaveMode As Int16 = 0
    ...
    Private Sub Form172_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Me.ComboBox2.SelectedItem = "2020"
    ...
    Private Sub ExportToExcelR1()
        ...
        Ws.Activate()
        LineZ = 4
        Ws.Cells(2, 12) = Today.ToString("yyyy/MM/dd")
        ReportYear = Me.ComboBox2.SelectedItem
    ...
    Private Sub ProcessStep6()
        ...
        If mSQLReader.HasRows() Then
            Dim SC As Int16 = 1
            While mSQLReader.Read()
                mSQLS11.CommandText = "INSERT INTO IED2 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(0) & "','包装Packing',1)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
    ...
    Public Sub CAllBom(ByVal g_bmb01 As String, ByVal bmb01 As String, ByVal Qty1 As Decimal)
        ...
        If oReader99.HasRows() Then
            While oReader99.Read()
                ...
                ' 要取得D0 的 工藝分類
                mSQLS11.CommandText = "Select Catalog from IED0 WHERE PN = '" & oReader99.Item(0) & "'"
                Dim l_Cata As String = mSQLS11.ExecuteScalar()

                mSQLS11.CommandText = "INSERT INTO IED2 VALUES ('" & g_bmb01 & "','" & oReader99.Item(0) & "','" & l_Cata & "'," & oReader99.Item(1) & ")"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
    ...
    Private Sub CheckedListBox2_ItemCheck(sender As Object, e As ItemCheckEventArgs) Handles CheckedListBox2.ItemCheck
        SingleSelectCheckedListBoxControls2(CheckedListBox2, e.Index)
    End Sub
    Public Sub SingleSelectCheckedListBoxControls2(ByVal CheckedListbox2 As CheckedListBox, ByVal s1 As Decimal)
        If CheckedListBox2.CheckedItems.Count > 0 Then
            For i As Int16 = 0 To CheckedListBox2.Items.Count - 1 Step 1
                If i <> s1 Then
    ...
    Private Sub ProcessStep0()
        Label3.Text = "处理IED0中"
        Label3.Refresh()
        mSQLS1.CommandText = "DELETE IED0 "
        Try
            mSQLS1.ExecuteNonQuery()
        Catch ex As Exception
            MsgBox(ex.Message())
        ...
        oCommand.CommandText = "Select ima01,ssa.tc_imf02,nvl(ima58,0),nvl(ssa.tc_imf03,0),nvl(ssa.tc_imf04,0),nvl(ssb.t1,0),nvl(ssb.t2,0),ssa.tc_imf05,ssa.tc_imf06,'',"
        Select Case EQindex
            Case 0
                oCommand.CommandText += "tc_imf05,"
            Case 1
                oCommand.CommandText += "(case when tc_imf06 = '5Axis' then tc_imf06 else tc_imf05 end),"
            Case 2
                oCommand.CommandText += "(case when tc_imf06 = '3Axis' then tc_imf06 else tc_imf05 end),"
        End Select
        oCommand.CommandText += "tc_imf07,tc_imf08,tc_imf09,nvl(tc_imf10,0) from ima_file "
        oCommand.CommandText += "left join tc_imf_file ssa on ima01 = ssa.tc_imf01 Left Join (Select substr(ssa.tc_imf01,0,15) as xx,nvl(sum(ima58),0) as t1,nvl(sum(tc_imf04),0) as t2 from ima_file "
        oCommand.CommandText += "left join tc_imf_file ssa on ima01 = ssa.tc_imf01 where length(ima01) > 14 group by substr(tc_imf01,0,15)) ssb on substr(ssa.tc_imf01,0,15) = ssb.xx "
        oCommand.CommandText += "where length(ima01) > 14 "
        If Math.Abs(CurrentWeek - StartWeek) > 10 Then
            oCommand.CommandText += "and imaud12 = 2 "
        End If
        oCommand.CommandText += "and ima01 not like 'S%' "

        oReader = oCommand.ExecuteReader()
        If oReader.HasRows() Then
            Dim RW As Integer = 1
            While oReader.Read()
                mSQLS1.CommandText = "INSERT INTO IED0 VALUES ('" & oReader.Item(0) & "','" & oReader.Item(1) & "'," & oReader.Item(2) & "," & oReader.Item(3) & "," & oReader.Item(4)
                mSQLS1.CommandText += "," & oReader.Item(5) & "," & oReader.Item(6) & ",'" & oReader.Item(7) & "','" & oReader.Item(8) & "',NULL,'" & oReader.Item(10) & "','" & oReader.Item(11)
                mSQLS1.CommandText += "','" & oReader.Item(12) & "','" & oReader.Item(13) & "'," & oReader.Item(14) & ")"
                Try
                    mSQLS1.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
                End Try
                Label3.Text = "处理IED0" & RW
                Label3.Refresh()
                RW += 1
            End While
        End If
    ...
    End Sub
    Private Sub CheckProcessStep0()
        Label3.Text = "检查IED0中"
        Label3.Refresh()
        mSQLS1.CommandText = "Select PN FROM IED0 WHERE IED0.Catalog is null  or IED0.Catalog = ''"
        mSQLReader = mSQLS1.ExecuteReader()
        If mSQLReader.HasRows() Then
            Label3.Text = "20001"
            Label3.Refresh()
            mSQLS11.CommandText = "UPDATE IEE1 SET Result = 'NG' WHERE ErrorCode = '20001'"
            Try
                mSQLS11.ExecuteNonQuery()
            Catch ex As Exception
                MsgBox(ex.Message())
            End Try
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IES4 WHERE PN = '" & mSQLReader.Item(0) & "' AND CDate = '" & Today.ToString("yyyy/MM/dd") & "' "
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception

                End Try
                mSQLS11.CommandText += "INSERT INTO IES4 (PN, CDate) VALUES ('" & mSQLReader.Item(0) & "','" & Today.ToString("yyyy/MM/dd") & "') "
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
            ...
            End While
        Else
            mSQLS11.CommandText = "UPDATE IEE1 SET Result = 'OK' WHERE ErrorCode = '20001'"
            Try
                mSQLS11.ExecuteNonQuery()
            Catch ex As Exception
                MsgBox(ex.Message())
    ...
    Private Sub Button10_Click(sender As Object, e As EventArgs) Handles Button10.Click
        ...
        StartWeek = TextBox1.Text
        EndWeek = TextBox2.Text
        AutoClaveMode = Me.ComboBox4.SelectedIndex
    ...
    Private Sub SaveExcelR5()
        SaveFileDialog1.FileName = "R5_Demand"
        SaveFileDialog1.DefaultExt = ".xlsx"
        Dim SON As DialogResult = SaveFileDialog1.ShowDialog()
        If SON = DialogResult.OK Then
            Dim SFN As String = SaveFileDialog1.FileName
    ...
    Private Sub ExportToExcelR5()
        ...
        Ws.Activate()
        LineZ = 2
        mSQLS1.CommandText = "Select WeekNo, PN1,PNB, Cutting1, Qty, StandardTime1 , CycleTime  from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '裁纱Cutting' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' "
        mSQLS1.CommandText += "union all "
        mSQLS1.CommandText += "Select WeekNo, IEC1.PN, '',isnull(Cutting1,0), 1, ies6.Time1 , ies6.time23   from iec1 left join IED1 on iec1.pn =  IED1.PN left join ies6 on iec1.pn = ies6.PN "
        mSQLS1.CommandText += "Where TempIETime is not null AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            Dim R1 As Int16 = 1
            While mSQLReader.Read()
                Ws.Cells(LineZ, 1) = R1
                Ws.Cells(LineZ, 2) = mSQLReader.Item(0)
                Ws.Cells(LineZ, 3) = mSQLReader.Item(1)
                Ws.Cells(LineZ, 4) = mSQLReader.Item(2)
                Ws.Cells(LineZ, 5) = mSQLReader.Item(3)
                Ws.Cells(LineZ, 6) = mSQLReader.Item(4)
                Ws.Cells(LineZ, 7) = mSQLReader.Item(5)
                Ws.Cells(LineZ, 8) = "=E" & LineZ & "*F" & LineZ & "*G" & LineZ & "/60"
                Ws.Cells(LineZ, 9) = mSQLReader.Item(6)
                Ws.Cells(LineZ, 10) = "=E" & LineZ & "*F" & LineZ & "*I" & LineZ & "/60"
                R1 += 1
                LineZ += 1
            End While
            ' 劃線
            oRng = Ws.Range("A2", Ws.Cells(LineZ - 1, 10))
        ...
        Ws.Activate()
        LineZ = 2
        mSQLS1.CommandText = "Select WeekNo, PN1,PNB, Layup1, Qty, StandardTime1 from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '预型Layup' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' "
        mSQLS1.CommandText += "union all "
        mSQLS1.CommandText += "Select WeekNo, IEC1.PN, '',isnull(Layup1,0), 1, ies6.Time3 from iec1 left join IED1 on iec1.pn =  IED1.PN left join ies6 on iec1.pn = ies6.PN "
        mSQLS1.CommandText += "Where TempIETime is not null AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            Dim R1 As Int16 = 1
            ...
            End While
            ' 劃線
            oRng = Ws.Range("A2", Ws.Cells(LineZ - 1, 8))
            oRng.Borders(xlEdgeLeft).LineStyle = xlContinuous
            oRng.Borders(xlEdgeTop).LineStyle = xlContinuous
            oRng.Borders(xlEdgeBottom).LineStyle = xlContinuous
            oRng.Borders(xlEdgeRight).LineStyle = xlContinuous
        ...
        LineZ = 2
        mSQLS1.CommandText = "Select WeekNo, PN1,PNB, Layup1, Qty, StandardTime1 from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'PCM Layup' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            Dim R1 As Int16 = 1
            While mSQLReader.Read()
        ...
        LineZ = 2
        mSQLS1.CommandText = "Select WeekNo, PN1,PNB, Layup1, Qty, StandardTime1 from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '结构件预型Structure Layup' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            Dim R1 As Int16 = 1
            While mSQLReader.Read()
        ...
        Ws.Activate()
        LineZ = 2
        mSQLS1.CommandText = "Select WeekNo, PN1,PNB, Molding1, Qty, StandardTime1, StandardTime2, PEQ, "
        If AutoClaveMode = 0 Then
            mSQLS1.CommandText += "Size,"
        Else
            mSQLS1.CommandText += "Weight,"
        End If
        mSQLS1.CommandText += "H1, CycleTime from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '成型Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' "
        mSQLS1.CommandText += "union all "
        mSQLS1.CommandText += "Select WeekNo, IEC1.PN, '',isnull(Molding1,0), 1, ies6.Time4, Time27, MNAME1, "
        If AutoClaveMode = 0 Then
            mSQLS1.CommandText += "Size1,"
        Else
            mSQLS1.CommandText += "Weight1,"
        End If
        mSQLS1.CommandText += "Cav1, Time26 from iec1 left join IED1 on iec1.pn =  IED1.PN left join ies6 on iec1.pn = ies6.PN "
        mSQLS1.CommandText += "Where TempIETime is not null AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            Dim R1 As Int16 = 1
            While mSQLReader.Read()
                ...
                Ws.Cells(LineZ, 9) = mSQLReader.Item(6)
                Ws.Cells(LineZ, 10) = "=E" & LineZ & "*F" & LineZ & "*I" & LineZ & "/60"
                Ws.Cells(LineZ, 11) = mSQLReader.Item(7)
                Ws.Cells(LineZ, 12) = mSQLReader.Item(8)
                Ws.Cells(LineZ, 13) = mSQLReader.Item(9)
                Ws.Cells(LineZ, 14) = mSQLReader.Item(10)
                Ws.Cells(LineZ, 15) = "=IF(K" & LineZ & "=""/"",0,E" & LineZ & "*F" & LineZ & "*L" & LineZ & "/M" & LineZ & "*N" & LineZ & ")/60"
                R1 += 1
                LineZ += 1
            End While
            ' 劃線
            oRng = Ws.Range("A2", Ws.Cells(LineZ - 1, 15))
            oRng.Borders(xlEdgeLeft).LineStyle = xlContinuous
            oRng.Borders(xlEdgeTop).LineStyle = xlContinuous
            oRng.Borders(xlEdgeBottom).LineStyle = xlContinuous
            oRng.Borders(xlEdgeRight).LineStyle = xlContinuous
        ...
        End If
        mSQLS1.CommandText += "H1, CycleTime from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'PCM Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            Dim R1 As Int16 = 1
            While mSQLReader.Read()
        ...
        End If
        mSQLS1.CommandText += "H1, CycleTime from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '结构件成型Structure Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            Dim R1 As Int16 = 1
            While mSQLReader.Read()
        ...
        End If
        mSQLS1.CommandText += "H1, CycleTime from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '乳胶芯材Latex Core' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            Dim R1 As Int16 = 1
            While mSQLReader.Read()
        ...
        Ws.Activate()
        LineZ = 2
        mSQLS1.CommandText = "Select WeekNo, PN1,PNB, CNC1, Qty, StandardTime1, StandardTime2, PEQ "
        mSQLS1.CommandText += "from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'CNC' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' "
        mSQLS1.CommandText += "union all "
        mSQLS1.CommandText += "Select WeekNo, IEC1.PN, '',isnull(CNC1,0), 1, ies6.Time7, Time28, MNAME2 "
        mSQLS1.CommandText += "from iec1 left join IED1 on iec1.pn =  IED1.PN left join ies6 on iec1.pn = ies6.PN "
        mSQLS1.CommandText += "Where TempIETime is not null AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            Dim R1 As Int16 = 1
            ...
            End While
            ' 劃線
            oRng = Ws.Range("A2", Ws.Cells(LineZ - 1, 11))
            oRng.Borders(xlEdgeLeft).LineStyle = xlContinuous
            oRng.Borders(xlEdgeTop).LineStyle = xlContinuous
            oRng.Borders(xlEdgeBottom).LineStyle = xlContinuous
            oRng.Borders(xlEdgeRight).LineStyle = xlContinuous
        ...
        Ws.Activate()
        LineZ = 2
        mSQLS1.CommandText = "Select WeekNo, PN1,PNB, Glueing1, Qty, StandardTime1, StandardTime2 "
        mSQLS1.CommandText += "from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '胶合Gluing' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' "
        mSQLS1.CommandText += "union all "
        mSQLS1.CommandText += "Select WeekNo, IEC1.PN, '',isnull(Glueing1,0), 1, ies6.Time9, Time29 "
        mSQLS1.CommandText += "from iec1 left join IED1 on iec1.pn =  IED1.PN left join ies6 on iec1.pn = ies6.PN "
        mSQLS1.CommandText += "Where TempIETime is not null AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
        ...
        Ws.Activate()
        LineZ = 2
        mSQLS1.CommandText = "Select distinct c.model , WeekNo, PN1,PNB, Sanding1, Qty, TimeTotal ,  StandardTime2 "
        mSQLS1.CommandText += "from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "left join IQMES3.dbo.model_station_paravalue c on c.profilename = 'ERP' and c.cf01 = PNB left join IED10 on c.model = ied10.ModelID "
        mSQLS1.CommandText += "where IED2.Catalog = '补土Sanding1' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and len(pnb) = 15 "
        mSQLS1.CommandText += "union all "
        mSQLS1.CommandText += "Select '' , WeekNo, IEC1.PN, '',isnull(Sanding1,0), 1, (Time11 + Time12 + Time13 + Time14), Time30 "
        mSQLS1.CommandText += "from iec1 left join IED1 on iec1.pn =  IED1.PN left join ies6 on iec1.pn = ies6.PN "
        mSQLS1.CommandText += "Where TempIETime is not null AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            ...
            While mSQLReader.Read()
                ...
                Ws.Cells(LineZ, 6) = mSQLReader.Item(5)
                Ws.Cells(LineZ, 7) = mSQLReader.Item(6)
                Ws.Cells(LineZ, 8) = "=E" & LineZ & "*F" & LineZ & "*G" & LineZ & "/60"
                Ws.Cells(LineZ, 9) = mSQLReader.Item(7)
                Ws.Cells(LineZ, 10) = "=E" & LineZ & "*F" & LineZ & "*I" & LineZ & "/60"
                R1 += 1
                LineZ += 1
            End While
        ...
        Ws.Activate()
        LineZ = 2
        mSQLS1.CommandText = "Select WeekNo, PN1,PNB, Painting1, Qty, PaintTime1, StandardTime2, CycleTime "
        mSQLS1.CommandText += "from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '涂装Painting4' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' "
        mSQLS1.CommandText += "union all "
        mSQLS1.CommandText += "Select WeekNo, IEC1.PN, '',isnull(Painting1,0), 1, (Time15 + Time16 + Time17 + Time18), Time31, Time22 "
        mSQLS1.CommandText += "from iec1 left join IED1 on iec1.pn =  IED1.PN left join ies6 on iec1.pn = ies6.PN "
        mSQLS1.CommandText += "Where TempIETime is not null AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            ...
            While mSQLReader.Read()
                ...
                Ws.Cells(LineZ, 8) = "=E" & LineZ & "*F" & LineZ & "*G" & LineZ & "/60"
                Ws.Cells(LineZ, 9) = mSQLReader.Item(6)
                Ws.Cells(LineZ, 10) = "=E" & LineZ & "*F" & LineZ & "*K" & LineZ & "/60"
                Ws.Cells(LineZ, 11) = mSQLReader.Item(7)
            ...
            End While
            ' 劃線
            oRng = Ws.Range("A2", Ws.Cells(LineZ - 1, 12))
            oRng.Borders(xlEdgeLeft).LineStyle = xlContinuous
            oRng.Borders(xlEdgeTop).LineStyle = xlContinuous
            oRng.Borders(xlEdgeBottom).LineStyle = xlContinuous
            oRng.Borders(xlEdgeRight).LineStyle = xlContinuous
        ...
        Ws.Activate()
        LineZ = 2
        mSQLS1.CommandText = "Select distinct c.model , WeekNo, PN1,PNB, Polishing1, Qty, IED11.StandardTime3, IED0.StandardTime2 "
        mSQLS1.CommandText += "from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "left join IQMES3.dbo.model_station_paravalue c on c.profilename = 'ERP' and c.cf01 = PNB left join IED11 on c.model = ied11.ModelID "
        mSQLS1.CommandText += "where IED2.Catalog = '抛光Polishing' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and len(pnb) = 15 "
        mSQLS1.CommandText += "union all "
        mSQLS1.CommandText += "Select '',WeekNo, IEC1.PN, '',isnull(Glueing1,0), 1, (Time19 + Time21), Time32 "
        mSQLS1.CommandText += "from iec1 left join IED1 on iec1.pn =  IED1.PN left join ies6 on iec1.pn = ies6.PN "
        mSQLS1.CommandText += "Where TempIETime is not null AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
        ...
        Ws.Activate()
        LineZ = 2
        mSQLS1.CommandText = "Select WeekNo, PN1,PNB, Package1, Qty, StandardTime1, StandardTime2 "
        mSQLS1.CommandText += "from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '包装Packing' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' "
        mSQLS1.CommandText += "union all "
        mSQLS1.CommandText += "Select WeekNo, IEC1.PN, '',isnull(Glueing1,0), 1, ies6.Time20, Time33 "
        mSQLS1.CommandText += "from iec1 left join IED1 on iec1.pn =  IED1.PN left join ies6 on iec1.pn = ies6.PN "
        mSQLS1.CommandText += "Where TempIETime is not null AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' order by WeekNo"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
    ...
    Private Sub ProcessStep0_1()
        Label3.Text = "处理D10中"
        Label3.Refresh()
        mSQLS1.CommandText = "DELETE IED10"
        Try
            mSQLS1.ExecuteNonQuery()
        Catch ex As Exception
        ...
        mSQLS1.CommandText = "Select Station1 FROM IES1 Where id = 11"

        Dim StationS1 As String = mSQLS1.ExecuteScalar()

        mSQLS1.CommandText = "INSERT INTO IED10 "
        mSQLS1.CommandText += "Select  Model , sum(t1) as t1,sum(t2) as t2,sum(t3) as t3,sum(t4) as t4,sum(t5) as t5,sum(t6) as t6,sum(t7) as t7,sum(t1+t2+t3+t4+t5+t6+t7)  from ( "
        mSQLS1.CommandText += "Select ab.Model , (case when StationGroupId = 5 then TaktTimeVal else 0 end) as t1,(case when StationGroupId = 6 then TaktTimeVal else 0 end) as t2,"
        mSQLS1.CommandText += "(case when StationGroupId = 8 then TaktTimeVal else 0 end) as t3,(case when StationGroupId = 9 then TaktTimeVal else 0 end) as t4,(case when StationGroupId = 10 then TaktTimeVal else 0 end) as t5,"
        mSQLS1.CommandText += "(case when StationGroupId = 12 then TaktTimeVal else 0 end) as t6,(case when StationGroupId = 11 then TaktTimeVal else 0 end) as t7 from IQMES3.dbo.routing aa "
        mSQLS1.CommandText += "left join IQMES3.dbo.model ab on aa.route = ab.default_route left join EFD.dbo.TaktTime ac on ab.model = ac.ModelId where aa.station = '" & StationS1 & "' and ab.model is not null and (ac.StationGroupId in (5,6,8,9,10,11,12) OR ac.StationGroupId is null) ) as AD group by Model "
        ...
        mSQLS1.CommandText = "Select ModelID from ied10 where TimeTotal = 0"
        mSQLReader = mSQLS1.ExecuteReader()
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "INSERT INTO IEList_S8 VALUES ('" & mSQLReader.Item(0) & "','D10_T8',NULL,NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        mSQLS1.CommandText = "Select Count(*) from IELIST_S8 A, IES8 B WHERE A.PN = B.PN AND A.Catalog = B.Catalog "
        Dim SBS As Int16 = mSQLS1.ExecuteScalar()
        If SBS > 0 Then
            mSQLS1.CommandText = "UPDATE IEE1 SET Result = 'NG' WHERE ErrorCode = '22001'"
            Try
                mSQLS1.ExecuteNonQuery()
            Catch ex As Exception
                MsgBox(ex.Message())
    ...
    Private Sub CleanData()
        mSQLS1.CommandText = "DELETE IEList_S8"
        Try
            mSQLS1.ExecuteNonQuery()
        Catch ex As Exception
    ...
    Private Sub ProcessStep0_2()
        Label3.Text = "处理D11中"
        Label3.Refresh()
        mSQLS1.CommandText = "DELETE IED11"
        Try
            mSQLS1.ExecuteNonQuery()
        Catch ex As Exception
        ...
        mSQLS1.CommandText = "Select Station1 FROM IES1 Where id = 13"

        Dim StationS1 As String = mSQLS1.ExecuteScalar()

        'mSQLS1.CommandText = "INSERT INTO IED11 "
        'mSQLS1.CommandText += "Select  ModelId , sum(t1) as t1,sum(t2) as t2,sum(t1+t2)  from ( "
        'mSQLS1.CommandText += "Select ModelID , (case when StationGroupId = 2 then TaktTimeVal else 0 end) as t1,(case when StationGroupId = 20 then TaktTimeVal else 0 end) as t2 "
        'mSQLS1.CommandText += "from efd.dbo.TaktTime Where StationGroupId in (2, 20)  ) as AB Group by ModelId "

        mSQLS1.CommandText = "INSERT INTO IED11 "
        mSQLS1.CommandText += "Select  Model , sum(t1) as t1,sum(t2) as t2,sum(t1+t2)  from ( "
        mSQLS1.CommandText += "Select ab.Model , (case when StationGroupId = 2 then TaktTimeVal else 0 end) as t1,(case when StationGroupId = 20 then TaktTimeVal else 0 end) as t2 "
        mSQLS1.CommandText += "from IQMES3.dbo.routing aa "
        mSQLS1.CommandText += "left join IQMES3.dbo.model ab on aa.route = ab.default_route left join EFD.dbo.TaktTime ac on ab.model = ac.ModelId where aa.station = '" & StationS1 & "' and ab.model is not null and (ac.StationGroupId in (2, 20) OR ac.StationGroupId is null) ) as AD group by Model "
        ...
        mSQLS1.CommandText = "Select ModelID from ied11 where StandardTime3 = 0"
        mSQLReader = mSQLS1.ExecuteReader()
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "INSERT INTO IEList_S8 VALUES ('" & mSQLReader.Item(0) & "','D11_T3',NULL,NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        Dim SBS As Int16 = mSQLS1.ExecuteScalar()
        If SBS > 0 Then
            mSQLS1.CommandText = "UPDATE IEE1 SET Result = 'NG' WHERE ErrorCode = '23001'"
            Try
                mSQLS1.ExecuteNonQuery()
            Catch ex As Exception
                MsgBox(ex.Message())
    ...
    Private Sub CheckR5()
        ...
        Label3.Refresh()
        ' 裁紗1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '裁纱Cutting' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception

                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','裁纱Cutting',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' 裁紗2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T5' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '裁纱Cutting' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (CycleTime =0 or CycleTime is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' 預型1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '预型Layup' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','预型Layup',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' PCM 預型1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'PCM Layup' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','PCM Layup',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' 結構件 預型1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '结构件预型Structure Layup' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','结构件预型Structure Layup',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' 成型1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '成型Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','成型Molding',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' 成型2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '成型Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' 成型3
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T5' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '成型Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (CycleTime =0 or CycleTime is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' 成型4
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_P1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '成型Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (PEQ is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' PCM 成型1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'PCM Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','PCM Molding',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' PCM 成型2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'PCM Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' PCM 成型3
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T5' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'PCM Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (CycleTime =0 or CycleTime is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' PCM 成型4
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_P1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'PCM Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (PEQ is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        '  结构件成型Structure Molding 1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '结构件成型Structure Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','结构件成型Structure Molding',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' 结构件成型Structure Molding2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '结构件成型Structure Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' 结构件成型Structure Molding 3
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T5' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '结构件成型Structure Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (CycleTime =0 or CycleTime is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' 结构件成型Structure Molding 4
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_P1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '结构件成型Structure Molding' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (PEQ is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        '  乳胶芯材Latex Core 1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '乳胶芯材Latex Core' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','乳胶芯材Latex Core',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' 乳胶芯材Latex Core 2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '乳胶芯材Latex Core' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' 乳胶芯材Latex Core 3
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T5' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '乳胶芯材Latex Core' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (CycleTime =0 or CycleTime is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' 乳胶芯材Latex Core 4
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_P1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '乳胶芯材Latex Core' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (PEQ is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' CNC1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'CNC' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','CNC',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' CNC2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'CNC' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' CNC3
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_P1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = 'CNC' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (PEQ is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' Glueing1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '胶合Gluing' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','胶合Gluing',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' Glueing2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '胶合Gluing' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' Sanding1
        mSQLS1.CommandText = "Select distinct PNB,'D10_T8' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "left join IQMES3.dbo.model_station_paravalue c on c.profilename = 'ERP' and c.cf01 = PNB left join IED10 on c.model = ied10.ModelID "
        mSQLS1.CommandText += "where IED2.Catalog = '补土Sanding1' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and len(pnb) = 15 and (TimeTotal = 0 OR TimeTotal is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','补土Sanding1',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' Sanding 2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '补土Sanding1' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' Painting1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_N1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '涂装Painting4' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (PaintTime1 =0 or PaintTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','涂装Painting4',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' Painting2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '涂装Painting4' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' Painting 3
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T5' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '涂装Painting4' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (CycleTime =0 or CycleTime is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' Polishing1
        mSQLS1.CommandText = "Select distinct PNB,'D11_T3' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "left join IQMES3.dbo.model_station_paravalue c on c.profilename = 'ERP' and c.cf01 = PNB left join IED10 on c.model = ied10.ModelID "
        mSQLS1.CommandText += "where IED2.Catalog = '抛光Polishing' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and len(pnb) = 15 and (StandardTime3 = 0 OR StandardTime3 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','抛光Polishing',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' Polishing 2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '抛光Polishing' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        ' Packing1
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T1' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '包装Packing' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime1 =0 or StandardTime1 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
                ...
                End Try
                mSQLS11.CommandText = "INSERT INTO IELIST_S8 VALUES ('" & mSQLReader.Item(0) & "','" & mSQLReader.Item(1) & "','包装Packing',NULL)"
                Try
                    mSQLS11.ExecuteNonQuery()
                Catch ex As Exception
                    MsgBox(ex.Message())
        ...
        ' Packing2
        mSQLS1.CommandText = "Select Distinct PNB, 'D0_T2' from ied2 left join IED1 ON IED2.PN1 = IED1.PN LEFT JOIN IED0 ON PNB = IED0.PN "
        mSQLS1.CommandText += "where IED2.Catalog = '包装Packing' AND WeekNo BETWEEN '" & StartWeek & "' AND '" & EndWeek & "' and (StandardTime2 =0 or StandardTime2 is null)"
        mSQLReader = mSQLS1.ExecuteReader
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                mSQLS11.CommandText = "DELETE IELIST_S8 WHERE PN = '" & mSQLReader.Item(0) & "' AND Catalog = '" & mSQLReader.Item(1) & "' "
        ...
        mSQLReader.Close()
    End Sub
    Private Sub CheckR5_1()
        Label3.Text = "R5检查错误中"
        Label3.Refresh()
        mSQLS1.CommandText = "Select * from IEList_S8 left join IES8 on IEList_S8.pn = ies8.PN and IEList_S8.Catalog = ies8.Catalog WHERE ies8.pn is null"
        mSQLReader = mSQLS1.ExecuteReader()
        If mSQLReader.HasRows() Then
            While mSQLReader.Read()
                ...
                Select Case mSQLReader.Item(1)
                    ...
                    Case "D0_T1"
                        Select Case mSQLReader.Item(2)
                            ...
                            Case "PCM Layup"
                                ErrorCode = "29003"
                            Case "结构件预型Structure Layup"
                                ErrorCode = "29004"
                            Case "成型Molding"
                                ErrorCode = "29005"
                            Case "PCM Molding"
                                ErrorCode = "29006"
                            Case "结构件成型Structure Molding"
                                ErrorCode = "29007"
                            Case "乳胶芯材Latex Core"
                                ErrorCode = "29008"
                            Case "CNC"
                ...
                If Not String.IsNullOrEmpty(ErrorCode) Then
                    mSQLS11.CommandText = "UPDATE IEE1 SET Result = 'NG' WHERE ErrorCode = " & ErrorCode
                    Try
                        mSQLS11.ExecuteNonQuery()
                    Catch ex As Exception
                        MsgBox(ex.Message())
