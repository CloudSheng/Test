...
Public Class Form58
    ...
    Private Sub SaveExcel()
        SaveFileDialog1.FileName = "DAC_AR_aging_report_" & DStartN.ToString("yyyyMMdd")
    ...
    Private Sub ExportToExcel()
        ...
        oReader.Close()
        ' 加總
        Ws.Cells(LineZ, 1) = "Total_forecast_AR_RMB"
        ...
        If oReader.HasRows() Then
            While oReader.Read()
                ...
                Dim SD As Date = IIf(IsDBNull(oReader.Item("oma09")), oReader.Item("oma02"), oReader.Item("oma09"))
                Dim SS As Decimal = DateDiff(DateInterval.Day, SD, DStartN)
        ...
        oReader.Close()
        ' 加總
        Ws.Cells(LineZ, 5) = "Total AR amount_RMB"
        ...
        Ws.Name = "AR overview"
        AdjustExcelFormat2()
        Ws.Cells(2, 2) = "='AR detail'!P" & LineZ
        Ws.Cells(3, 2) = "='AR detail'!O" & LineZ
        Ws.Cells(4, 2) = "='AR detail'!N" & LineZ
        Ws.Cells(5, 2) = "='AR detail'!M" & LineZ
        Ws.Cells(6, 2) = "='AR detail'!L" & LineZ
        Ws.Cells(7, 2) = "='AR detail'!K" & LineZ
        Ws.Cells(8, 2) = "=SUM(B2:B7)"
        ...
        XShape.Chart.ChartTitle.Text = "how many days since invoice date"
        ...
        oCommand.CommandText += C1 & " AND alz03 = " & C2 & " AND alz04 = oma01  group by oma69"
        oReader = oCommand.ExecuteReader()
        If oReader.HasRows() Then
            While oReader.Read()
        ...
        XShape2.Chart.ChartTitle.Text = "AR amount by customer"
        ' 第四頁   20160829
        Ws = xWorkBook.Sheets(4)
        Ws.Activate()
        Ws.Name = "Customer VS Invoice days"
        AdjustExcelFormat3()
        oCommand.CommandText = "SELECT oma69,sum(t1) as t1,sum(t2) as t2,sum(t3) as t3,sum(t4) as t4,sum(t5) as t5,sum(t6) as t6 from ("
        oCommand.CommandText += "SELECT oma69,(case when to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02)  <= 9 then sum(alz09) end) as t1,(case when to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02) >= 10 and to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02) < 30 then sum(alz09) end) as t2,"
        oCommand.CommandText += "(case when to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02) >= 30 and to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02) < 60 then sum(alz09) end) as t3,(case when to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02) >= 60 and to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02) < 90 then sum(alz09) end) as t4,"
        oCommand.CommandText += "(case when to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02) >= 90 and to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02) < 120 then sum(alz09) end) as t5,(case when to_date('" & DStartN.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - nvl(oma09,oma02) >= 120 then sum(alz09) end) as t6  "
        oCommand.CommandText += "FROM OMA_FILE,alz_file WHERE alz09 > 0 and oma03 = alz01 AND alz00 = '2' AND alz02 = "
        oCommand.CommandText += C1 & " AND alz03 = " & C2 & " AND alz04 = oma01 group by oma69,oma09,oma02 order by oma69 ) group by oma69"
        oReader = oCommand.ExecuteReader()
        If oReader.HasRows() Then
            While oReader.Read()
                Ws.Cells(LineX, 1) = oReader.Item("oma69")
    ...
    Private Sub AdjustExcelFormat1()
        ...
        Ws.Cells(1, 10) = "how many days since invoice date"
        Ws.Cells(1, 11) = "less than 9 days"
        Ws.Cells(1, 12) = "over 10 days"
        Ws.Cells(1, 13) = "over 30 days"
        oRng = Ws.Range("M1", "M1")
    ...
    Private Sub AdjustExcelFormat2()
        ...
        oRng.EntireColumn.NumberFormatLocal = "_ [$￥-804]* #,##0.00_ ;_ [$￥-804]* -#,##0.00_ ;_ [$￥-804]* ""-""??_ ;_ @_ "
        Ws.Cells(1, 1) = "how many days since invoice date"
        Ws.Cells(1, 2) = "sum amount"
        Ws.Cells(2, 1) = "over 120 days"
        Ws.Cells(3, 1) = "over 90 days"
        Ws.Cells(4, 1) = "over 60 days"
        Ws.Cells(5, 1) = "over 30 days"
        Ws.Cells(6, 1) = "over 10 days"
        Ws.Cells(7, 1) = "less than 9 days"
        Ws.Cells(8, 1) = "Total AR_USD"
        oRng = Ws.Range("A8", "B8")
        oRng.Interior.Color = Color.Yellow
        Ws.Cells(10, 1) = "customer"
        Ws.Cells(10, 2) = "amount"
        LineX = 11
    End Sub
    Private Sub AdjustExcelFormat3()
        ...
        'Ws.Cells(1, 2) = "sum amount"
        Ws.Cells(1, 7) = "over 120 days"
        Ws.Cells(1, 6) = "over 90 days"
        Ws.Cells(1, 5) = "over 60 days"
        Ws.Cells(1, 4) = "over 30 days"
        Ws.Cells(1, 3) = "over 10 days"
        Ws.Cells(1, 2) = "less than 9 days"
