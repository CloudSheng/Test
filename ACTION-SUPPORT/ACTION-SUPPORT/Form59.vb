...
Public Class Form59
    ...
    Private Sub SaveExcel()
        SaveFileDialog1.FileName = "ACA_AR_aging_report_" & TimeS1.ToString("yyyyMMdd")
    ...
    Private Sub ExportToExcel()
        'Dim TimeS1 As DateTime = Me.DateTimePicker1.Value.ToString("yyyy/MM/dd")
        ...
        For i As Integer = 0 To DS.Tables("ACAAR").Rows.Count - 1 Step 1
            If IsDBNull(DS.Tables("ACAAR").Rows(i).Item(0)) Then
                Continue For
            End If
            If DS.Tables("ACAAR").Rows(i).Item("invoice_date") > TimeS1 Then
                Continue For
            End If
        ...
        For i As Integer = 0 To DS.Tables("table2").Rows.Count - 1 Step 1
            If IsDBNull(DS.Tables("table2").Rows(i).Item(0)) Then
                Continue For
            End If
            If DS.Tables("table2").Rows(i).Item("invoice_date") > TimeS1 Then
                Continue For
            End If
            ...
            Dim DD2 As Decimal = DateDiff(DateInterval.Day, Convert.ToDateTime(DS.Tables("table2").Rows(i).Item("invoice_date")), TimeS1)
        ...
        Ws.Name = "AR overview"
        AdjustExcelFormat2()
        Ws.Cells(2, 2) = "='AR detail'!P" & LineZ + 1
        Ws.Cells(3, 2) = "='AR detail'!O" & LineZ + 1
        Ws.Cells(4, 2) = "='AR detail'!N" & LineZ + 1
        Ws.Cells(5, 2) = "='AR detail'!M" & LineZ + 1
        Ws.Cells(6, 2) = "='AR detail'!L" & LineZ + 1
        Ws.Cells(7, 2) = "='AR detail'!K" & LineZ + 1
        ...
        For i As Integer = 0 To DS.Tables("table2").Rows.Count - 1 Step 1
            ...
            oCommand.CommandText += "'," & DS.Tables("table2").Rows(i).Item("due_balance") * ER & ",to_date('" & DS.Tables("table2").Rows(i).Item("invoice_date") & "','yyyy/mm/dd'))"
            Try
                oCommand.ExecuteNonQuery()
            Catch ex As Exception
                MsgBox(ex.Message())
        ...
        oCommand.CommandText += "(Case when to_date('" & TimeS1.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - idate <= 9 then aamount end) as t0,"
        oCommand.CommandText += "(Case when to_date('" & TimeS1.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - idate >= 10 and to_date('" & TimeS1.ToString("yyyy/MM/dd") & "','yyyy/MM/dd') - idate < 30 then aamount end) as t1,"
    ...
    Private Sub AdjustExcelFormat()
        ...
        oCommand.CommandText += TimeS1.ToString("yyyy/MM/dd") & "','yyyy/mm/dd') order by azn02,azn05"
        oReader = oCommand.ExecuteReader()
        If oReader.HasRows() Then
            CC = 0
            While oReader.Read()
    ...
    Private Sub AdjustExcelFormat1()
        ...
        oRng = Ws.Range("M1", "M1")
        oRng.Interior.Color = Color.LightGreen
        Ws.Cells(1, 14) = "over 60 days"
        oRng = Ws.Range("N1", "N1")
        oRng.Interior.Color = Color.Yellow
        Ws.Cells(1, 15) = "over 90 days"
        oRng = Ws.Range("O1", "O1")
        oRng.Interior.Color = Color.Orange
        Ws.Cells(1, 16) = "over 120 days"
        oRng = Ws.Range("P1", "P1")
        oRng.Interior.Color = Color.Red
        oRng = Ws.Range("K1", "P1")
        oRng.EntireColumn.NumberFormatLocal = "_-[$€-2] * #,##0.00_-;-[$€-2] * #,##0.00_-;_-[$€-2] * ""-""??_-;_-@_-"
